...
*/
	...
	*/
	public function salvar($_data, $pula = false){
		//echo "<pre>"; print_r($_data); "</pre>";
		$c = new Conta();
		$c->where('num_conta',$_data['num_conta'])->get();
		if(!$c->exists()){
			$_data['saldo'] = 0;
			$_data['saldo'] = isset($_data['saldo'])? $_data['saldo']: 0;
			$this->num_conta         = $_data['num_conta'];
			...
			if(!$pula){
				$_data['saldo'] = isset($_data['saldo'])? $_data['saldo']: 0;
				$c->where('num_conta', $_data['num_conta'])->update(array('saldo' => 'saldo'.$_data['saldo']));	
				return $c;
			}
			else{
				return $c;
	...
	public function getById($id){
		$c = new Conta();
		return $c->where('num_conta',$id)->get();
	}

	public function sacar($id, $valor = 0){
		$c2 = $this->getById($id);
		$valor = $c2->stored->saldo - $valor;
		$c = new Conta();
		$c->where('num_conta', $id)->update(array('saldo' => $valor));
		return $this->getById($id);
	}
	
	public function depositar($id, $valor = 0){
		$c2 = $this->getById($id);
		$valor = $c2->stored->saldo + $valor;
		$c = new Conta();
		$c->where('num_conta', $id)->update(array('saldo' => $valor));
	...
	public function transferir($_data){
		$this->depositar($_data['origem'], $_data['valor']);
		return $this->sacar($_data['num_conta'], $_data['valor']);
	}
